...
if (!$property) {
    ...
    $pageTitle = 'Không tìm thấy bất động sản - ' . SITE_NAME;
    require_once __DIR__ . '/includes/header.php';
    echo '<div class="container"><p class="empty-state">Không tìm thấy bất động sản bạn đang tìm.</p></div>';
  ...
  <div class="detail-layout">
    <div>
      <div class="detail-gallery">
        <img id="detailMainImage" src="<?= e(imageUrl($images[0])) ?>" alt="<?= e($property['title']) ?>">
      </div>
      <?php if (count($images) > 1): ?>
      <div class="gallery-thumbs">
        <?php foreach ($images as $i => $img): ?>
          <img src="<?= e(imageUrl($img)) ?>" class="gallery-thumb <?= $i === 0 ? 'active' : '' ?>" alt="">
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
      ...
      <div class="spec-grid">
        <div class="spec-item">
          ...
          <strong><?= e(rtrim(rtrim(number_format((float)$property['area'], 1), '0'), '.')) ?> m²</strong>
          <span>Diện tích</span>
        </div>
        <div class="spec-item">
      ...
      <div class="detail-section">
        ...
        <p><?= nl2br(e($property['description'] ?: 'Chưa có mô tả cho bất động sản này.')) ?></p>
        <p style="color:var(--text-muted);font-size:13.5px;margin-top:10px;">Loại bất động sản: <strong><?= e($property['type_name']) ?></strong> &nbsp;|&nbsp; Mã tin: #<?= (int)$property['id'] ?> &nbsp;|&nbsp; <?= (int)$property['views'] ?> lượt xem</p>
      </div>
    ...
    <aside class="agent-card">
      <?php if ($property['agent_id']): ?>
        <div class="agent-head">
          <?php if ($property['agent_avatar']): ?>
            <img class="agent-avatar" src="<?= e(imageUrl($property['agent_avatar'])) ?>" alt="<?= e($property['agent_name']) ?>" style="object-fit:cover;">
          <?php else: ?>
            <div class="agent-avatar"><?= e($agentInitial) ?></div>
          <?php endif; ?>
          <div>
            <h3><?= e($property['agent_name']) ?></h3>
            <div class="agent-role">Nhân viên phụ trách</div>
          </div>
        </div>
        <p style="font-size:13.5px;color:var(--text-muted);margin:0 0 4px;">Liên hệ trực tiếp để được tư vấn và xem nhà:</p>
      ...
      <?php else: ?>
        ...
        <p class="no-agent">Bất động sản này chưa được phân công nhân viên phụ trách. Vui lòng liên hệ hotline 0900 000 000.</p>
      <?php endif; ?>
    </aside>
  </div>
</div>
